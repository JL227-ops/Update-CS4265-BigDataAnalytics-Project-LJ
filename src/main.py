...
from integration.integrate_sources import (
    ...
    aggregate_commoncrawl_signals,
    integrate_with_commoncrawl,
)
...
def log_count(label, df):
    ...
    logger.info(f"{label}: {count_value:,} records")
    return count_value
...
def main():
    ...
    try:
        ...
        product_trend_signals = integrate_with_trends(product_signals, trends_clean)
        product_trend_signals_count = log_count(
            "Product signals + Google Trends",
            product_trend_signals,
        )

        logger.info("Creating Common Crawl web mention signals")
        commoncrawl_signals = aggregate_commoncrawl_signals(commoncrawl_clean)
        commoncrawl_signals_count = log_count(
            "Common Crawl web signals",
            commoncrawl_signals,
        )

        logger.info("Integrating Common Crawl web signals into final output")
        final_output = integrate_with_commoncrawl(
            product_trend_signals,
            commoncrawl_signals,
        )
        ...
        save_with_logging(
            ...
            "Final M3 output with external signals",
        )
        ...
        logger.info(
            "M3 summary: reviews=%s, metadata=%s, trends=%s, commoncrawl=%s, "
            "integrated=%s, product_signals=%s, trends_joined=%s, "
            "commoncrawl_signals=%s, final=%s",
            f"{reviews_raw_count:,}",
            ...
            f"{product_trend_signals_count:,}",
            f"{commoncrawl_signals_count:,}",
